...
if ($_POST['do'] == 'doupload')
{
	...
	{
		$smiliePrefix = vB::getDatastore()->getOption('smiliepath');
	}

	if (empty($smiliePrefix))
	{
		$smiliePrefix = '';
	}
	...
	if (file_exists( './' . $smiliePrefix . $vbulletin->GPC['imagespath'] . '/' . $vbulletin->GPC['upload']['name']) OR
		file_exists(DIR . '/' . $smiliePrefix . $vbulletin->GPC['imagespath'] . '/' . $vbulletin->GPC['upload']['name']))
	{
	...
	$upload->path = $smiliePrefix . $vbulletin->GPC['imagespath'];
	...
	$fixpath = false;
	if (!is_dir($smiliePrefix . $vbulletin->GPC['imagespath']) )
	{
		$location = DIR . '/' . $smiliePrefix . $vbulletin->GPC['imagespath'] ;
