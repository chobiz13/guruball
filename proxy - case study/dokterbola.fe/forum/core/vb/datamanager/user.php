...
class vB_DataManager_User extends vB_DataManager
{
	...
	*/
	var $validfields = array(
		...
		'autosubscribe'      => array(vB_Cleaner::TYPE_INT,        vB_DataManager_Constants::REQ_NO, 'if (!in_array($data, array(0, 1))) { $data = 0; } return true;'),
	...
	*/
	function set_userfields(&$values, $verify = true, $all_fields = 'normal', $skip_unset_required_fields = false)
	{
		...
		switch($all_fields)
		{
			...
			case 'normal':
			default:
				$all_fields_sql = array('profilefieldid' => $field_ids, 'editable' => array(1));
				break;
		}
		...
		foreach ($profilefields as $profilefield)
		{
			...
			if ($all_fields != 'admin' AND !empty($this->existing["$varname"]))
			{
				if($profilefield['editable'] == 0 OR $profilefield['editable'] == 2)
				{
					throw new vB_Exception_Api('cannot_change_custom_field');
				}
			}
