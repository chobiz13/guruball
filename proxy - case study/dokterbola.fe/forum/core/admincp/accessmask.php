...
define('CVS_REVISION', '$RCSfile$ - $Revision: 87198 $');
...
if ($_REQUEST['do'] == 'edit')
{
	...
	print_form_header('admincp/accessmask', 'update');
...
if ($_REQUEST['do'] == 'quickedit')
{
	...
	print_form_header('admincp/accessmask', 'doquickedit');
	print_table_header($vbphrase['access_masks_quick_editor'], 4);
	print_cells_row(array(
		"<a href=\"admincp/accessmask.php?" . vB::getCurrentSession()->get('sessionurl') . "do=quickedit&amp;orderby=user\" title=\"" . $vbphrase['sort'] . "\">" . $vbphrase['username'] . "</a>",
		"<a href=\"admincp/accessmask.php?" . vB::getCurrentSession()->get('sessionurl') . "do=quickedit&amp;orderby=channel\" title=\"" . $vbphrase['sort'] . "\">" . $vbphrase['channel'] . "</a>",
	...
	if($result AND $result->valid())
	{
		while($result->valid())
		{
			...
			print_cells_row(array(
				"<a href=\"admincp/user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=editaccess&u=$access[userid]\">$access[username]</a>",
				"<a href=\"admincp/accessmask.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;n=$access[nodeid]&amp;accessmask=2\">$access[node_title]</a>",
				"
					<label for=\"rb_1_$radioname\"><input type=\"radio\" name=\"$radioname\" value=\"1\" id=\"rb_1_$radioname\" tabindex=\"1\"" . iif($sel==1," checked=\"checked\"","")." />" . $vbphrase['yes'] . "</label>
					<label for=\"rb_0_$radioname\"><input type=\"radio\" name=\"$radioname\" value=\"0\" id=\"rb_0_$radioname\" tabindex=\"1\"" . iif($sel==0," checked=\"checked\"","")." />" . $vbphrase['no'] . "</label>
					<label for=\"rb_x_$radioname\"><input type=\"radio\" name=\"$radioname\" value=\"-1\"  id=\"rb_x_$radioname\"tabindex=\"1\"" . iif($sel==-1," checked=\"checked\"","")." />" . $vbphrase['default'] . "</label>
...
{
	print_form_header('admincp/accessmask', 'doresetall');
...
function print_channels($parentid = 0, $indent = "	")
{
	...
	foreach ($channels AS $nodeid => $node)
	{
		...
		echo "$indent<li><b><a name=\"node$nodeid\" href=\"admincp/forum.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;n=$nodeid\">$node[htmltitle]</a> <span class=\"smallfont\">(" . construct_link_code($vbphrase['reset'], "accessmask.php?" . vB::getCurrentSession()->get('sessionurl') . "do=resetchannel&amp;n=$nodeid&amp;hash=" . CP_SESSIONHASH) . ")</span></b>";
...
if ($_REQUEST['do'] == 'modify')
{
	...
	print_description_row("<b><a href=\"admincp/accessmask.php?" . vB::getCurrentSession()->get('sessionurl') . "do=resetall\">" . $vbphrase['delete_all_access_masks'] . "</a> | <a href=\"admincp/accessmask.php?" . vB::getCurrentSession()->get('sessionurl') . "do=quickedit\">" . $vbphrase['access_masks_quick_editor'] . "</a></b>", 0, 2, '', 'center');
	print_table_footer();
