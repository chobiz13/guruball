...
function print_cp_login($mismatch = false)
{
	...
	$focusfield = ($vbulletin->userinfo['userid'] == 0 ? 'username' : 'password');
	...
	<table class="tborder" cellpadding="0" cellspacing="0" border="0" width="450" align="center"><tr><td>
		...
		if ($vbulletin->GPC['loginerror'])
		{
			...
			?>
			<tr>
				<td colspan="2" class="navbody error"><b><?php echo construct_phrase($errorphrase, vB5_Route::buildUrl('lostpw|fullurl'), $vbulletin->GPC['strikes']); ?></b></td>
			</tr>
			<?php
		}
		?>
		...
		<tr>
			...
			switch( intval($options['logintype']) )
			{
				...
				case 2:
					// both
					echo $vbphrase['username_or_email'];
					break;
				default:
					// should not happen.
					break;
		...
		<tr>
			...
			<td><input type="password" autocomplete="off" style="padding-<?php echo vB_Template_Runtime::fetchStyleVar('left'); ?>:5px; font-weight:bold; width:250px" name="vb_login_password" accesskey="p" tabindex="2" id="vb_login_password" /></td>
			<td>&nbsp;</td>
		</tr>
...
* @param 	string 	The base url (should be to the site root).  If blank will use the url set in the options.
*/
function print_cp_header($title = '', $onload = '', $headinsert = '', $marginwidth = 0, $bodyattributes = '', $base = '')
{
	...
	// update the debug mode warning in the head frame on all page loads
	...
	<script type="text/javascript">
		$(function()
		{
			...
			if (window && window.parent && window.parent.frames && window.parent.frames.length && window.parent.frames['head'])
			{
				if (isDebug)
				{
					$('.js-debug-warning-message', window.parent.frames['head'].document).show();
				}
				else
				{
					$('.js-debug-warning-message', window.parent.frames['head'].document).hide();
				}
			}
		});
	</script>
...
function construct_help_button($option = '', $action = NULL, $script = '', $helptype = 0, $helpOptions = array())
{
	...
	{
		case 1:
		return "<a id=\"$id\" class=\"helplink\" href=\"#\" onclick=\"$helplink\">$vbphrase[help] <img src=\"core/cpstyles/" . $vbulletin->options['cpstylefolder'] . "/cp_help." . $vbulletin->options['cpstyleimageext'] . "\" alt=\"\" border=\"0\" title=\"$vbphrase[click_for_help_on_these_options]\" style=\"vertical-align:middle\" /></a>";

		default:
		return "<a id=\"$id\" class=\"helplink\" href=\"#\" onclick=\"$helplink\"><img src=\"core/cpstyles/" . $vbulletin->options['cpstylefolder'] . "/cp_help." . $vbulletin->options['cpstyleimageext'] . "\" alt=\"\" border=\"0\" title=\"$vbphrase[click_for_help_on_this_option]\" /></a>";
	}
}
...
function construct_link_code($text, $url, $newwin = false, $tooltip = '', $smallfont = false, $admincp = true)
{
	...
	return ($smallfont ? '<span class="smallfont">' : '') . " <a href=\"" . $prefix . $url . "\"" . ($newwin ? " target=\"$newwin\"" : '') . (!empty($tooltip) ? " title=\"$tooltip\"" : '') . '>' . (vB_Template_Runtime::fetchStyleVar('textdirection') == 'rtl' ? "[$text&lrm;]</a>&rlm; " : "[$text]</a> ") . ($smallfont ? '</span>' : '');
}
