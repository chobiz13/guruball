...
class vB_Image_ImageMagick extends vB_Image
{
	...
	*/
	public function __construct($options)
	{
		...
		$this->magic_numbers[$soi] = array('eoi' => '', 'type' => "PDF");
		$two = substr($soi, 0, 2);
	...
	* @param	boolean   $autoorient   Automatically rotate/flip image if 'Orientation' exif data exists.
	*
	* @return	mixed
	*
	*/
	protected function fetchConvertedImage($filename, $output, $imageinfo, $thumbnail = true, $sharpen = true, $autoorient = false)
	{
		/*
			Prevent un-whitelisted files from being sent into imagick. Imagetragick mitigation.
			The only caller ATM already does this, but better to be safe in case something else calls this in the future.
...
#			}
	...
	/**
	 * @see vB_Image::orientImageInternal()
	 * @access protected
	 */
	protected function orientImageInternal($fileContents, $location, $orientation, $newfile)
	{
		...
		if (extension_loaded('imagick'))
		{
			try
			{
				$angles = $this->orientationToAnglesCW($orientation);
				$image = new Imagick($location);
				$check = $image->rotateImage(new ImagickPixel('#00000000'), $angles);
				if (!$check)
				{
				...
				if (!$check)
				{
					// file write failed for some reason.
					return "";
				}
				else
				{
			...
			{
				// let's let the default commandline try it if something failed above...
			}
		}

		// Commandline implementation if imagick class isn't there. Just take advantage of -auto-orient which will take the exif data and use it to rotate it.
		$imageinfo = $this->fetchImageInfo($location);
		$this->setConvertOptions(0, 0, 100, false, false, false, null, null, null);
		$write = $this->fetchConvertedImage($location, $newfile, $imageinfo, false, false, true);
		if ($write)
		{
			return $newfile;
		}
