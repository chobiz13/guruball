...
define('CVS_REVISION', '$RCSfile$ - $Revision: 88257 $');
define('NOZIP', 1);
...
if ($_REQUEST['do'] == 'modify')
{
	...
	function construct_banned_user_row($user, $canunbanuser)
	{
		...
		$cell[] = construct_link_code(
			...
			'modcp/banning.php?' . vB::getCurrentSession()->get('sessionurl') . "do=editreason&amp;userid=" . $user['userid'],
			false,
			'',
			false,
			false
	...
	if (!$havebanned)
	{
		if ($canbanuser)
		{
			print_stop_message('no_users_banned_from_x_board_click_here', '<b>' . $vbulletin->options['bbtitle'] . '</b>', 'modcp/banning.php?' . vB::getCurrentSession()->get('sessionurl') . 'do=banuser');
		}
		else
		{
