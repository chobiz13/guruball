...
define('CVS_REVISION', '$RCSfile$ - $Revision: 88940 $');
...
if ($_REQUEST['do'] == 'remove')
{
	...
	echo '<p align="center">' . construct_phrase($vbphrase['if_you_want_to_prune_user_posts_first'], "thread.php?do=pruneuser&amp;f=-1&amp;u=" . $vbulletin->GPC['userid'] . "&amp;confirm=1") . '</p>';
}
...
if ($_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'add')
{
	...
	if ($vbulletin->GPC['userid'])
	{
		...
		{
			$groups = vB_Library::instance('user')->fetchUserGroups($vbulletin->GPC['userid']);
		...
		$quicklinks = array(
			"admincp/user.php?do=editaccess&u=" . $vbulletin->GPC['userid']
				=> $vbphrase['edit_forum_permissions_access_masks'],
			"admincp/resources.php?do=viewuser&u=" . $vbulletin->GPC['userid']
		...
		{
			$quicklinks[
				"admincp/subscriptions.php?do=adjust&amp;userid=" . $vbulletin->GPC['userid']
		...
		if (
			intval($user['adminpermissions']) & $vbulletin->bf_ugp_adminpermissions['cancontrolpanel'] AND
			in_array($user['userid'], preg_split('#\s*,\s*#s', $vb5_config['SpecialUsers']['superadmins'], -1, PREG_SPLIT_NO_EMPTY))
		)
		{
			$quicklinks["admincp/adminpermissions.php?do=edit&u=" . $vbulletin->GPC['userid']] = $vbphrase['edit_administrator_permissions'];
		}
...
/*	$user = $vbulletin->db->query_first("SELECT username, options FROM " . TABLE_PREFIX . "user WHERE userid = " . $vbulletin->GPC['userid']);
	...
	print_form_header('admincp/user', 'updateaccess');
	construct_hidden_code('userid', $vbulletin->GPC['userid']);
