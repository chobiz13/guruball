...
define('CVS_REVISION', '$RCSfile$ - $Revision: 89025 $');
...
if ($_REQUEST['do'] == 'domoveattachment')
{
	...
	foreach ($filedataQry as $filedata)
	{
		...
		if ($vb5_config['Misc']['debug'])
		{
			...
			echo "	<tr>
					<td>$filedata[filedataid]" . iif($attacherror, "<br />$attacherror") . "</td>
...
if ($_REQUEST['do'] == 'search')
{
	...
	print_table_header(
		...
		($vbulletin->GPC['pagenum'] - 1) * $vbulletin->GPC['search']['results'] + 1,
		iif(
			$vbulletin->GPC['search']['results'] * $vbulletin->GPC['pagenum'] > $attachments['count'],
			$attachments['count'],
			$vbulletin->GPC['search']['results'] * $vbulletin->GPC['pagenum']
		),
		$attachments['count']),
		7
	);
	...
	$perPage = $vbulletin->GPC['search']['results'];
	...
	foreach ($attachments AS $attachment)
	{
		...
		$cell[] = iif($attachment['userid'], "<a href=\"admincp/user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;u=$attachment[userid]\">$attachment[username]</a>", $attachment['username']);
...
if ($_REQUEST['do'] == 'stats')
{
	...
	if ($attachments = vB::getDbAssertor()->assertQuery('vBForum:fetchTopAttachmentsCounter'))
	{
		...
		foreach ($attachments AS $attachment)
		{
			...
			$cell[] = iif($attachment['userid'], "<a href=\"admincp/user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;u=$attachment[userid]\">$attachment[authorname]</a>", $attachment['authorname']);
	...
	//while($thispop = $vbulletin->db->fetch_array($largestuser))
	{
		...
		$cell = array();
		$cell[] = $position . '.';
		$cell[] = "<a href=\"admincp/user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;u=$thispop[userid]\">$thispop[username]</a>";
...
if ($_REQUEST['do'] == 'intro')
{
	...
	<ul style=\"margin:0px; padding:0px; list-style:none\">
		<li><a href=\"admincp/attachment.php?" . vB::getCurrentSession()->get('sessionurl') . "do=search&amp;search[orderby]=fd.filesize&amp;search[ordering]=DESC\">" . $vbphrase['view_largest_attachments'] . "</a></li>
		<li><a href=\"admincp/attachment.php?" . vB::getCurrentSession()->get('sessionurl') . "do=search&amp;search[orderby]=a.counter&amp;search[ordering]=DESC\">" . $vbphrase['view_most_popular_attachments'] . "</a></li>
		<li><a href=\"admincp/attachment.php?" . vB::getCurrentSession()->get('sessionurl') . "do=search&amp;search[orderby]=fd.dateline&amp;search[ordering]=DESC\">" . $vbphrase['view_newest_attachments'] . "</a></li>
		<li><a href=\"admincp/attachment.php?" . vB::getCurrentSession()->get('sessionurl') . "do=search&amp;search[orderby]=fd.dateline&amp;search[ordering]=ASC\">" . $vbphrase['view_oldest_attachments'] . "</a></li>
	</ul>
	");
	print_table_break();
