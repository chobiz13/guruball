...
//this branch didn't work going back to at least m4.  The if(...getRow...) line didn't
//assign the return value to anything and thus the $imageInfo variable was undefined
//this leads me to believe that this branch is unused and can perhaps be removed.
//that will require more research (made the obvious fix just is case but have
...
else if ($vbulletin->GPC['groupid'])
{
	...
	header('Cache-control: max-age=31536000');
	header('Expires: ' . gmdate('D, d M Y H:i:s', (TIMENOW + 31536000)) . ' GMT');
	if ($imageInfo = vB::getDbAssertor()->getRow('fetchSocialgroupIcon', $params))
	{
		header('Content-disposition: inline; filename=' . $imageinfo['filename']);
		header('Content-transfer-encoding: binary');
		header('Content-Length: ' . strlen($imageinfo['filedata']));
